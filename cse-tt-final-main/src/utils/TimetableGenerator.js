...
export const generateClassTimetable = (semester, section, rawSubjects, reservedSlots = {}, syncElectives = {}, relaxed = false, globalLabUsage = {}, slotsCount = 7, globalFacultyLoad = {}, semesterLabSlots = {}, year = 1) => {
    const SLOTS = slotsCount;
    const yearOffset = (year - 1) % SLOTS;
    ...
    uniqueSubjects.forEach(code => {
        ...
        for (const d of dOrder) {
            const shiftedSlot = (0 + yearOffset) % SLOTS;
            if (grid[d][shiftedSlot]) continue;
            
            const teachers = sub.allTeachers || (sub.teacherName !== 'TBA' ? String(sub.teacherName).split('/') : []);
            if (teachers.some(t => reservedSlots[`${d}-${shiftedSlot}`] && reservedSlots[`${d}-${shiftedSlot}`].has(String(t).trim().toUpperCase()))) continue;
            
            grid[d][shiftedSlot] = { ...sub, duration: 1, isStart: true };
            if (usedSlotsBySubject[sub.code]) usedSlotsBySubject[sub.code].add(shiftedSlot);
            if (isElective(sub)) {
                if (!syncElectives[sub.code]) syncElectives[sub.code] = [];
                syncElectives[sub.code].push({ d, s: shiftedSlot });
            }
